...
def plot_hours_history(filename,type):
    ...
    dfname = filename
    data = pd.read_csv(dfname, sep=r'\s+')
...
for f in glob.glob('*_usage_history*_[0-9]*.dat'):
    print(f)
...
for f in glob.glob('*_job_sizes*_[0-9]*.dat'):
    print(f)
...
for f in glob.glob('*_job_waits*_[0-9]*.dat'):
    print(f)
...
for f in glob.glob('*_pie_*_*_[0-9]*.dat'):
    print(f)
    plot_usage_pie(f)
...
def plot_job_durations(filename):
    ...
    if "job_durations" in filename:
        plttitle = '{} job durations\n{} to {}'.format(hrtype,date_start,date_end)
    elif "memory_per_rank" in filename:
        plttitle = '{} Memory per Rank\n{} to {}'.format(hrtype,date_start,date_end)
    else:
        print("Unhandled filename:", filename)
        assert False
...
for f in glob.glob('*_job_durations*_[0-9]*.dat'):
    print(f)
    plot_job_durations(f)

for f in glob.glob('*_job_memory_per_rank*_[0-9]*.dat'):
    print(f)
    plot_job_durations(f)

...
| `job_history/queries/jobs.py` | `JobQueries` class — high-level query API; `jobs_search`/`jobs_count`/`jobs_facets`/`jobs_histogram`/`jobs_usage_by`/`jobs_timeseries` all share `_apply_jobs_search_filters` (no defaults there, on purpose); histogram bucket tables live on `QueryConfig` as `(label, lo, hi)` triples — closed + contiguous, enforced at import by `_validate_bucket_table` (an un-closed table mislabels rows *silently*, leaving `total_count` correct); `nodes`/`cpus`/`gpus` are right-sized per machine by truncating the shared table at `QueryConfig.MACHINE_HIST_CAPS`, so closure holds by construction and an unlisted machine (or `machine="all"`) keeps the default (derecho is uncapped by measurement, not omission — 2487/318336/328 maxima all sit in the existing overflow bands); the module-level `histogram_buckets(dimension, machine=None)` is the **public** form of that selection and is what `jobs_histogram` itself calls, so a consumer building a filter control on it cannot drift from the axis the query bins on; every aggregate carries `_METRIC_KEYS` (hours **and** QoS-weighted `*_charges`, see `_charge_expr`). `jobs_timeseries` has a **`daily_summary` fast path** (~800× — `_timeseries_uses_summary` routes, `_timeseries_from_summary` executes); it falls back to scanning `jobs` for any filter the rollup aggregated away, or a window past the summary watermark |

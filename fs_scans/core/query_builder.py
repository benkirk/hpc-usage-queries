...
class DirectoryQueryBuilder:
    ...
    def with_path_prefix_anc(self, pairs: list[tuple[int, int]]) -> "DirectoryQueryBuilder":
        ...
        recursive descendants CTE, each scope ``(dir_id, depth)`` becomes an
        ``s.anc_d{depth} = :scope`` equality (OR'd together). The ``directories``
        ...
        Args:
            pairs: ``(dir_id, depth)`` per scope; every ``depth`` already
                verified ``<= SCOPE_MAX_DEPTH`` by the caller.
        ...
        preds = []
        for i, (dir_id, depth) in enumerate(pairs):
            key = f"anc_scope_{i}"
            self._params[key] = dir_id
            preds.append(f"s.anc_d{depth} = :{key}")
